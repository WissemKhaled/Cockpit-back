...
public class SubcontractorServiceImpl implements SubcontractorService {
	...
	public List<SubcontractorDto> getAllNonArchivedSubcontractors(String sortingMethod, int pageNumber, int pageSize) {
		...
		Optional<List<Subcontractor>> optionalSubcontractorsList = Optional
				.ofNullable(subcontractorMapper.findAllNonArchivedSubcontractors(sortingMethod, offset, pageSize));

		if (optionalSubcontractorsList.isEmpty()) {
			throw new EntityNotFoundException("Il n'y a pas de sous-traitans enregistré");
		}

		List<SubcontractorDto> foundSubcontractorList = optionalSubcontractorsList.get().stream()
	...
	@Override
	public List<SubcontractorDto> getAllSubcontractorWithStatus(String sortingMethod, int pageSize, int pageNumber,
			int statusId) {
		int offset = (pageNumber - 1) * pageSize;
		Optional<List<Subcontractor>> optionalSubcontractorsList;
		if (statusId == 0) {
			optionalSubcontractorsList = Optional
					.ofNullable(subcontractorMapper.findAllNonArchivedSubcontractors(sortingMethod, offset, pageSize));
		} else {
			optionalSubcontractorsList = Optional.ofNullable(
					subcontractorMapper.findAllSubcontractorsWithStatus(sortingMethod, offset, pageSize, statusId));
		}
	...
	public Integer getNumberOfAllSubcontractorsWithStatus(Integer statusId) {
		Integer numberOfFoundSubcontractorsWithStatus = subcontractorMapper
				.countAllNonArchivedSubcontractorsWithStatus(statusId);
	...
	public SubcontractorDto getSubcontractorWithStatus(int sId) {
		Optional<Subcontractor> optionalSubcontractor = Optional
				.ofNullable(subcontractorMapper.findSubcontractorWithStatusById(sId));
	...
	public int archiveSubcontractor(SubcontractorDto subcontractorDtoToArchive) {
		...
		List<ServiceProvider> foundedServiceProvidersBySubcontractorId = serviceProviderMapper
				.findServiceProvidersBySubcontractorId(subcontractorDtoToArchive.getSId());
	...
	public Subcontractor getSubcontractorBySName(String sName) {
		Subcontractor foundSubcontractorBySName = subcontractorMapper
				.findSubcontractorWithStatusBySubcontractorName(sName);
	...
	public List<SubcontractorDto> getAllSubcontractorsBySearchAndWithOrWithoutStatusFiltring(String searchTerms,
			int pageNumber, int pageSize, int statusId, String columnName, String sortingMethod)
		...
		if (statusId == 0) {
			subcontractorList = subcontractorMapper.findAllSubcontractorsByCriteria(criteriaColumn, searchTerms, offset,
					pageSize, sortingMethod);
		} else {
			subcontractorList = subcontractorMapper.findAllSubcontractorsByCriteriaAndFiltredByStatus(criteriaColumn,
					searchTerms, offset, pageSize, sortingMethod, statusId);
		}
		...
		setAlertsForSubcontractorDtosList(subcontractorDtoList);
	...
	public Integer getNumberOfSubcontractorsBySearchAndWithOrWithoutStatusFiltring(String searchTerms, int statusId,
		...
		if (statusId == 0) {
			...
			return subcontractorMapper.countAllSubcontractorsByCriteriaAndFiltredByStatus(criteriaColumn, searchTerms,
					statusId);
		}
	}
	...
	public int getPageNumberOfNewlyAddedOrUpdatedSubcontractor(int savedSubcontractorId, int pageSize) {
		...
		List<SubcontractorDto> sortedSubcontractors = subcontractorMapper
				.findAllNonArchivedSubcontractors("asc", 0, countAllNonArchivedServiceProviders).stream()
				.map(subcontractorDtoMapper::subcontractorToDto).toList();
		...
		if (newIndex != -1) {
			// calculer le nombre de page en se basant sur l'indice et le nombre d'élément
			// par page.
	...
	private void setAlertsForSubcontractorDtosList(List<SubcontractorDto> subcontractorDtos) {
		for (SubcontractorDto subcontractorDto : subcontractorDtos) {
			Optional<List<Integer>> countAllSubcontractorAlerts = Optional
					.ofNullable(subcontractorMapper.countAllSubcontractorsAlerts(subcontractorDto.getSId()));
