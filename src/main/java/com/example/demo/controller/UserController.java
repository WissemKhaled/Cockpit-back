...
public class UserController {
	...
	/**
	 * Méthode de création d'un user avec validation du pattern de mot de passe et
	 * email
	 */
	...
	/**
	 * Méthode qui vérifie l'authentification du User et génère un token avec un
	 * tokenId servant à le refresh si l'authentification réussie
	 */
	...
	public ResponseEntity<JwtResponseDTO> authenticateAndGetToken(@RequestBody AuthRequest authRequest) {
		try {
			...
			Authentication authentication = authenticationManager
					.authenticate(new UsernamePasswordAuthenticationToken(authRequest.getEmail(), decodedPwd));

			// on vérifie que l'utilisateur a un status actif. Si c'est le cas, on génère un
			// token
			...
			if (authentication.isAuthenticated() && user.isUStatus()) {
				...
				// on supprime la clé de refresh token associée à l'utilisateur s'il y en a déjà
				// une en bdd avant d'en créer une
				...
				log.error("Requête utilisateur invalide ou utilisateur '" + authRequest.getEmail() + "' inactif !");
				throw new UsernameNotFoundException("Requête utilisateur invalide ou utilisateur inactif '"
						+ authRequest.getEmail() + "' inactif !");
			}
	...
	/**
	 * Méthode qui intercèpte les exceptions de validation (pattern du mdp et de
	 * l'email...)
	 */
