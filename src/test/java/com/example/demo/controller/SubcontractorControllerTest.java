...
public class SubcontractorControllerTest {
	...
	@BeforeEach
	void init() {
		Subcontractor savedSubcontractorForTesting = new Subcontractor(2, "Orange",
				"Orange@email.fr", new Status(1, "EN_COURS", "AAAA"));
	...
	public void testGetSubcontractor_SubcontractorFound() throws Exception {
		...
		mvc.perform(MockMvcRequestBuilders.get(baseUrl + subcontractorId).contentType(MediaType.APPLICATION_JSON))
				...
				.andExpect(jsonPath("$.sId").value(subcontractorId))
				.andExpect(jsonPath("$.sName").value(expectedName))
				.andExpect(jsonPath("$.sEmail").value(expectedEmail))
	...
	public void testSaveSubcontractor_saveNewSubcontractor() throws Exception {
		Subcontractor savedSubcontractor = new Subcontractor(1, "test_saving",
				"test_saving@email.fr", new Status(1, "EN_COURS", "AAAA"));

		mvc.perform(MockMvcRequestBuilders.post(baseUrl + "/save")
				.content(asJsonString(savedSubcontractor))
