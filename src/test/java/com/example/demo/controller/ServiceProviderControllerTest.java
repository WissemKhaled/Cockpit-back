...
//		.andExpect(jsonPath("$.spStatus.stId").value(3))
	...
	public void testUpdateServiceProvider_UpdateExistingServiceProvider_ShouldReturnHttpOk() throws Exception {
		...
		existingServiceProviderTosave.setSubcontractorSName("BPCE");
		existingServiceProviderTosave.setSpStatus(new Status(1));
		...
		ServiceProviderForUpdate.setSubcontractorSName("Orange");
		ServiceProviderForUpdate.setSpStatus(new Status(1));
		...
		mockMvc.perform(MockMvcRequestBuilders.post(baseUrl + "save").content(asJsonString(ServiceProviderForUpdate))
				...
				.andExpect(jsonPath("$.subcontractorSName").value("Orange")).andExpect(jsonPath("$.spStatus.stId").value(1));
	}
