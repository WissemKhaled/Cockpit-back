...
public class ServiceProviderDtoMapperTest {
	...
	void whenConvertServiceProviderEntityToServiceProviderDto_thenCorrect() {
		...
		assertEquals(serviceProviderEntity.getSpStatus().getStId(), serviceProviderDto.getSpStatus().getStId());
		assertEquals(serviceProviderEntity.getSubcontractor().getSName(), serviceProviderDto.getSubcontractorSName());
	}
	...
	void whenConvertServiceProviderDtoToServiceProviderEntity_thenCorrect() {
		...
		serviceProviderDto.setSpStatus(new Status(1));
		serviceProviderDto.setSubcontractorSName("Orange");
		...
		assertEquals(serviceProviderDto.getSpStatus().getStId(), serviceProvider.getSpStatus().getStId());
		assertEquals(serviceProviderDto.getSubcontractorSName(), serviceProvider.getSubcontractor().getSName());
	}
}
